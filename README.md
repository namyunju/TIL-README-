...
<details><summary>250908greedy</summary>

greedy: 결정이 필요할 때 현재 기준으로 가장 좋아보이는 선택지로 결정하여 답을 도출하는 알고리즘

dp: 현재에서 가장 좋아보이는 것 선택이 아니라, 과거 데이터 이용해 현재 데이터를 만들어내는 문제 해결 기법

이후 분할정복 배움: 큰 문제를 작은 문제로 나누어 해결하는 기법
...
가장 큰 동전부터 고르면 최소 동전 수가 나온다 참일까?

최적해보다 더 적은 동전으로 표현이 가능하다 가정. N원을 거 작은 동전으로 
동전이 배수로 있기 떄문에 안됨. 더 작은 동전 이용시 무조건 더 큰 개수의 동전이 필요함

그리디 사용 가능 조건
1. 조건 바꾸면 안됨
2. 부분 최적해가 전체 최적해가 되어야 함
3. (반례 없)
